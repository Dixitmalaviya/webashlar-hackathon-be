...
export class RecordService {
  static async createRecord(payload) {
    const { patient, doctor, hospital, diagnosis, treatment, prescription, notes, consentScope } = payload;
    const draft = { 
      patient, 
      doctor, 
      hospital, 
  ...
  static async getRecord(id) {
    const record = await MedicalRecord.findById(id).populate('patient doctor hospital report');
